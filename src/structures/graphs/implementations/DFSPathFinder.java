...
public class DFSPathFinder<T> implements PathFinder<T> {
    ...
    public PathResult<T> find(Graph<T> graph, T start, T end) {
        // Implementación del algoritmo DFS para encontrar el camino entre start y end
        // en el grafo
    ...
    private boolean dfs(Graph<T> graph,
        ...
        // Llamada recursiva para cada vecino del nodo actual
        for (Node<T> vecino : graph.getVecinos(current)) {
            if (!visited.contains(vecino.getValue())) {
                boolean encontrado = dfs(graph,
                        vecino.getValue(), end,
                        visited, path);
                if (encontrado) {
                    return true;
            ...
            path.remove(path.size() - 1); // Retroceder si no se encuentra el camino
        }
        return false;
